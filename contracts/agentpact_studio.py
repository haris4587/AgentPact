...
"""Studionet demonstration of evidence-bound agent work adjudication.

Rewards and awards are nominal test figures. No GEN is received or transferred.
Use agentpact.py on a network with value transfers for actual escrow.
"""
...
class AgentPactStudio(gl.Contract):
    ...
    def create_job(self, title: str, brief: str, criteria_json: str,
                   repo: str, deadline: u256, challenge_seconds: u256,
                   reward_gen: u256) -> None:
        ...
        if reward_gen == u256(0) or reward_gen > u256(1000000):
            raise gl.vm.UserError("Demo reward must be 1 to 1,000,000 GEN units")
        job_id = self.count
        job = {"id": int(job_id), "buyer": str(gl.message.sender_address),
               ...
               "reward_wei": str(reward_gen * u256(10**18)), "demo_only": True,
               "worker_award_wei": "0", "buyer_remainder_wei": "0", "evidence_url": "",
    ...
    def settle(self, job_id: u256) -> None:
        ...
        nominal_reward = int(job["reward_wei"])
        worker_award = nominal_reward * job["payout_bps"] // 10000
        job["worker_award_wei"] = str(worker_award)
        job["buyer_remainder_wei"] = str(nominal_reward - worker_award)
        job["status"] = "SETTLED"
        self.jobs[job_id] = json.dumps(job)
